...
文件把动量列 (p~x/p~y) 归一化到 **mc**、能量列 (E_kin) 也都归一化到 **mc** (均无量纲;
即坐标序 scale=[1, mc, 1, mc, 1, mc], 交叉项乘 mc 一次 — 能量列不是除以 mc^2):
...
(批 5 澄清: 列本身均除以 mc, 而非能量列除以 mc^2; 之前多处文字写 除以 mc^2 与自身公式矛盾, 代码 scale=[1,mc,1,mc,1,mc] 正确。)
